...
class AIVizGenerator {
  ...
  interpretPromptLocally(prompt) {
    ...
    // Refine bar variants (horizontal / stacked / grouped)
    if (result.chartType === "bar") {
      if (prompt.includes("horizontal")) {
        result.chartType = "horizontalBar";
      } else if (prompt.includes("stacked")) {
        result.chartType = "stackedBar";
      } else if (
        prompt.includes("grouped") ||
        prompt.includes("side by side") ||
        prompt.includes("clustered")
      ) {
        result.chartType = "groupedBar";
      }
    }
  ...
  generateChartConfig(aiResult) {
    ...
    const datasets = processedData.datasets.map((dataset, i) => {
      ...
      if (isBarVariant && processedData.datasets.length === 1) {
        const colors = getColorsForCategories(processedData.labels);
        return {
          label: dataset.label,
          data: dataset.data,
    ...
    const hasScales = isBarVariant || chartType === "line";
    ...
    // Extra options for bar variants
    ...
    if (chartType === "groupedBar") {
      options.scales.x = { ...(options.scales.x || {}), stacked: false };
      options.scales.y = { ...(options.scales.y || {}), stacked: false };
    }
    ...
    try {
      ...
      if (typeof setChartType === "function") {
        ...
        setTimeout(() => {
          ...
          const calcEl = document.getElementById("kpiCalculation");
          const titleEl = document.getElementById("kpiTitle");

          if (!valueEl || !calcEl || !titleEl) {
            showAIStatus("❌ KPI controls not found in the page", "error");
            return;
          }

          // Now safely set the UI fields after the KPI tab is ready
          valueEl.value = kpiCol;
