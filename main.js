...
function isCurrencyColumn(columnName) {
  ...
  // Only columns that clearly suggest money get $ (avoid "total" / "value" – too generic)
  ...
  return currencyKeywords.some((kw) => lower.includes(kw));
}
...
function formatKPIValue(value, calculation, valueColumn) {
  ...
  const useCurrency = isCurrencyColumn(valueColumn);

  const formatNum = (n, opts = {}) => n.toLocaleString(undefined, opts);
  ...
  if (absValue >= 1000000) {
    ...
    return `${prefix}${formatNum(numValue)}`;
  } else {
    return `${prefix}${formatNum(numValue, {
